...
if (!isset($_REQUEST['q']))
    $result = Konekcija::dajInstancu()->izvrsiUpit(
        "SELECT * FROM proizvodi LEFT JOIN dobavljaci on proizvodi.DOB_ID=dobavljaci.DOB_ID  
                                 WHERE proizvodi.vracen IS NULL or proizvodi.vracen=0
                                 ORDER BY proizvodi.PROIZ_DatumIsteka ASC");
else
    $result = Konekcija::dajInstancu()->izvrsiUpit(
        "SELECT * FROM proizvodi LEFT JOIN dobavljaci on proizvodi.DOB_ID=dobavljaci.DOB_ID
                                 WHERE (proizvodi.vracen IS NULL or proizvodi.vracen=0) AND (PROIZ_Naziv LIKE \"" . $_REQUEST['q'] . "%\" or DOB_Naziv LIKE \"" . $_REQUEST['q'] . "%\") 
                                 ORDER BY proizvodi.PROIZ_DatumIsteka ASC");
